...
var GlobalNode = function(globalName, subscripts) {
  ...
  this._getDocument = function() {
    ...
    var getSubnodes = function(globalNode) {
      ...
      do {
        ...
        result = node.result;
        if (result !== '') {
          //subs2 = node.subscripts.slice(0);
          //gnode = {global: globalName, subscripts: subs2};
          //defined = ewd.mumps.data(gnode).defined;
          defined = ewd.mumps.data(node).defined;
          if (defined === 1 || defined === 11) {
            document[result] = ewd.mumps.get(node).data;
          }
          if (defined === 10 || defined === 11) {
            var subDocument = getSubnodes(node);
            document[result] = subDocument;
          }
        }
      }
      while (result !== '');
  ...
  this._setDocument = function(document, fast) {
