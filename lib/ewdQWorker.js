  ...
  cacheActionMethod: function(action) {
    ...
    if (action.exit) {
      ...
      if (action.type === 'onWebService') {
        ...
        var auth = new mumps.GlobalNode('zewd', ['webServiceAccessId', action.query.accessId]);
        if (!auth._exists) return {error: 'Invalid security credentials'};
        var secretKey = auth._value;
