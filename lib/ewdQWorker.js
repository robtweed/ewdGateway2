...
var EWD = {
  ...
  getSessid: function(token) {
    ...
    var sessid = cache.get(node).data;
  ...
  getMemory: function() {
    ...
    var zewd = new mumps.GlobalNode('%zewd', ['childProcessMemory', process.pid]);
    zewd._setDocument(memory);
    memory.pid = process.pid;
    memory.modules = ewdQWorker.module;
    return memory;
  },
  isTokenExpired: function(token) {
    ...
    var expiry = +cache.get(node).data;
    ewdSession = null;
    token = null;
    sessid = null;
  ...
  createNewSession: function(application, timeout) {
    ...
    return {sessid: sessid, token: token};
  },
  getSession: function(token) {
    if (this.isTokenExpired(token)) {
      ...
      ewdSession.sessid = cache.get({global: '%zewdSession', subscripts: ['session', sessid, 'ewd_sessid']}).data;
      ewdSession.app = cache.get({global: '%zewdSession', subscripts: ['session', sessid, 'ewd_appName']}).data;
      //ewdSession.app = ewdSession.$('ewd_appName')._value;
      ewdSession.page = cache.get({global: '%zewdSession', subscripts: ['session', sessid, 'ewd_pageName']}).data;
      //ewdSession.page = ewdSession.$('ewd_pageName')._value;
      ewdSession.isAuthenticated = (+cache.get({global: '%zewdSession', subscripts: ['session', sessid, 'ewd_authenticated']}).data === 1);
      //ewdSession.isAuthenticated = (+ewdSession.$('ewd_authenticated')._value === 1);
      ewdSession.setAuthenticated = function() {
        cache.set({global: '%zewdSession', subscripts: ['session', sessid, 'ewd_authenticated'], data: 1})
  ...
  updateSessionExpiry: function(session) {
    ...
    var timeout = +cache.get({global: '%zewdSession', subscripts: ['session', sessid, 'ewd_sessionTimeout']}).data;
    //var timeout = +session.$('ewd_sessionTimeout')._value;
    var expiry = now + timeout;
    //session.$('ewd_sessionExpiry')._value = expiry;
    cache.set({global: '%zewdSession', subscripts: ['session', sessid, 'ewd_sessionExpiry'], data: expiry})
  }, 
  ...
  deleteExpiredSessions: function() {
    ...
    sessions._forEach(function(sessid, session) {
      ...
      ewdQWorker.log("sessid: " + sessid + "; expiry: " + expiry + "; now: " + EWD.hSeconds(), 3);
  ...
  requireAndWatch: function(path) {
    var module = false;
    try {
      module = require(path);
      //console.log("requireAndWatch: " + path + " loaded");
      fs.watch(require.resolve(path), function(event, filename) {
        ewdQWorker.log(filename + ' has changed - event = ' + event, 3);
        if (event === 'change') {
          delete require.cache[require.resolve(path)];
          try {
            var module = require(path);
            ewdQWorker.log(path + " reloaded successfully", 3);
          }
          catch (err) {
            ewdQWorker.log(path + " could not be reloaded: " + JSON.stringify(err), 3);
          }
        }
      });
      ...
      ewdQWorker.log("Error in requireAndWatch - " + path + " could not be loaded", 3);
    }
    return module;
  }
...
var ewdQWorker = {
  ...
  cacheActionMethod: function(action) {
    ...
    if (ewdQWorker.traceLevel > 2) ewdQWorker.log("Action method: Process " + process.pid + ": action = " + JSON.stringify(action),3);
    var result = '';
    var resultObj;
    var funcObj;
    if (action.exit) {
    ...
    if (action.getMemory) {
      ...
      if (action.type === 'onWebService') {
        ...
        ewdQWorker.log("stringToSign: " + stringToSign, 3);
        var hash = EWD.digest(stringToSign, secretKey, type);
        ewdQWorker.log("hash: " + hash, 3);
        ewdQWorker.log("signature: " + action.query.signature, 3);
        if (hash !== action.query.signature) {
          // try a second time, not including the port with the host this time
          ewdQWorker.log("2nd attempt: ", 3);
        ...
        if (typeof ewdQWorker.module[action.appName] === 'undefined') {
          ...
          try {
            ...
            zewd = null;
            ewdSession = null;
            ewdQWorker.log("Unable to load module " + action.appName + ' at path ' + path + "; error = " + JSON.stringify(err), 1); 
        ...
        var params = {
          ...
          mumps: mumps,
          util: EWD,
          invokeWrapper: EWD.invokeWrapperFunction
        };
		if (ewdQWorker.database.type === 'mongodb') params.mongoDB = mongoDB;
        try {
      ...
      if (action.type === 'onBeforeRender') {
        ...
        if (ewdQWorker.traceLevel > 1) ewdQWorker.log(process.pid + ": onBeforeRender method request - " + JSON.stringify(action), 2);
        if (ewdQWorker.traceLevel > 1) ewdQWorker.log("onBeforeRender started at " + ewdQWorker.elapsedTime(), 2);
        ...
        if (action.lite) {
          ...
          path = path + delim + action.appName + '.js';
          //console.log("**** module path = " + path);
          if (action.messageType === 'EWD.getFragment') {
            var fragPath = ewdQWorker.webServerRootPath + '/ewd/' + action.handlerModule + '/' + action.params.file;
            var content = '';
            if (fs.existsSync(fragPath)) content = fs.readFileSync(fragPath, 'utf8');
            return {
              content: content,
              targetId: action.params.targetId,
              file: action.params.file,
              extra: action.params.extra,
            };
          }
        }
        else {
          zewd = new mumps.Global('zewd');
        ...
        var node = {global: 'zewd', subscripts: ['reloadRequire', process.pid, action.module]};
        //if (zewd.$('reloadRequire').$(process.pid).$(action.module)._value !== '') {
        if (cache.get(node).data !== '') {
        ...
        if (action.messageType === 'reloadModule') {
        ...
        else if (typeof ewdQWorker.module[action.module][action.method] === 'undefined' && !(action.method === 'onSocketMessage' && ewdQWorker.module[action.module].onMessage && ewdQWorker.module[action.module].onMessage[action.messageType])) {
          ...
          try {
            ...
            params.util = EWD;
            params.ipAddress = action.ipAddress;
            params.homePath = ewdQWorker.homePath;
			if (ewdQWorker.database.also.length > 0 && ewdQWorker.database.also[0] === 'mongodb') {
            ...
            */
            ...
            //
            var result;
            if (action.method === 'onSocketMessage' && ewdQWorker.module[action.module].onMessage && ewdQWorker.module[action.module].onMessage[action.messageType]) {
              result = ewdQWorker.module[action.module].onMessage[action.messageType](action.params, params);
            }
            else {
              result = ewdQWorker.module[action.module][action.method](params) || '';
            }
            //
            //
            ewdSession = null;
            if (ewdQWorker.traceLevel > 1) ewdQWorker.log("onBeforeRender completed at " + ewdQWorker.elapsedTime(), 2);
            return result;
          }
          catch(err) {
            zewd = null;
      ...
      if (action.type === 'initialise') {
		...
		if (ewdQWorker.database.type === 'mongodb') ewdQWorker.database.nodePath = 'mongoGlobals';
        ...
        else if (ewdQWorker.database.type === 'mongodb') {
		  mongo = require('mongo');
          mongoDB = new mongo.Mongo();
          cache = new globals.Mongo();
          cache.open(mongoDB, {address: ewdQWorker.database.address, port: ewdQWorker.database.port});
          ewdQWorker.database.namespace = '';
        }
		if (ewdQWorker.database.also.length > 0) {
		  if (ewdQWorker.database.also[0] === 'mongodb') {
		    mongo = require('mongo');
		    mongoDB = new mongo.Mongo();
			mongoDB.open({address: ewdQWorker.database.address, port: ewdQWorker.database.port});
		  }
		}
        mumps.init(cache);
        // ********************** Load Global Indexer *******************
        try {
          ...
          indexer.start(mumps);
          ewdQWorker.log("attempting to get the globalIndexer path....", 3);
        ...
        if (action.params.no === 1) {
          ...
          pczewd.$('childProcessMemory')._delete();
          pczewd = null;
          ...
          if (ewdQWorker.lite) zewd.$('nodeWorkers').$(action.params.httpPort)._delete();
    ...
    if (action.type === 'EWD.register') {
      ...
      var now = EWD.hSeconds();
      //var timeout = 3600;
      var timeout = action.application.timeout || 3600;
      var expiry = now + timeout;
    ...
    if (action.type === 'EWD.getSessionTokens') {
      var tokenArray = [];
      var sessions = new mumps.GlobalNode('%zewdSession', ['session']);
      sessions._forEach(function(sessid, sessionNode) {
        if (action.recipients === 'byApplication') {
          if (sessionNode.$('ewd_appName')._value === action.application) tokenArray.push(sessionNode.$('ewd_wstoken')._value);
        }
        if (action.recipients === 'bySessionValue') {
          if (action.session && (action.session instanceof Array)) {
            var ok = true;
            for (var i = 0; i < action.session.length; i++) {
              ok = ok && (sessionNode.$(action.session[i].name)._value === action.session[i].value);
            }
            if (ok) tokenArray.push(sessionNode.$('ewd_wstoken')._value);
          }
        }
        if (action.recipients === 'all') tokenArray.push(sessionNode.$('ewd_wstoken')._value);
      });
      return {
        tokens: tokenArray, 
