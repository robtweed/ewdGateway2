/*
 ...
 |                                                                          |
 | Copyright (c) 2012-14 M/Gateway Developments Ltd,                        |
 | Reigate, Surrey UK.                                                      |
 | All rights reserved.                                                     |
 |                                                                          |
...
var ewdQ = {
  ...
  addToQueue: function(requestObj, responseHandler) {
    ...
    if (ewdQ.traceLevel > 0) ewdQ.sendMgrMessage({type: 'queueInfo', qLength: qLength});
    if (qLength > ewdQ.maxQueueLength) ewdQ.maxQueueLength = qLength;
    if (ewdQ.traceLevel > 1) ewdQ.log("Request " + requestNo + " added to Queue: queue length = " + qLength + "; requestNo = " + ewdQ.totalRequests + "; after " + ewdQ.elapsedTime() + " sec", 2);
  ...
  memoryUsed: function() {
    ...
    for (var pid in ewdQ.process) {
      ewdQ.getChildProcessMemory(pid);
    }
    mem = null;
    pid = null;
  ...
  getChildProcess: function() {
    ...
    for (pid in ewdQ.process) {
      if (ewdQ.process[pid].isAvailable) {
        ewdQ.process[pid].isAvailable = false;
        if (ewdQ.traceLevel > 0) ewdQ.sendMgrMessage({type: 'pidUpdate', pid: pid, noOfRequests: ewdQ.requestsByProcess[pid], available: ewdQ.process[pid].isAvailable});
        return pid;
      }
    }
    return false;
  ...
  processQueue: function() {
    ...
    if (ewdQ.queue.length > 0)  {
      ewdQ.queueEvents++;
      if (ewdQ.traceLevel > 0) ewdQ.log("processing queue: " + ewdQ.queueEvents + "; queue length " + ewdQ.queue.length + "; after " + ewdQ.elapsedTime() + " seconds", 2);
      var queuedRequest;
      var pid;
      ...
      var unprocessed = [];
      if (!ewdQ.lite) for (var i = 0; i < ewdQ.queue.length; i++) {
        queuedRequest = ewdQ.queue[i];
        if (typeof queuedRequest.action.usePid !== 'undefined') {
          pid = queuedRequest.action.usePid;
          ewdQ.log("usePid = " + pid, 3);
          if (ewdQ.process[pid]) {
            ewdQ.log("checking if " + pid + " is available", 3);
            if (ewdQ.process[pid].isAvailable) {
              ...
              ewdQ.log("dispatching request " + requestNo + " to " + pid + "at " + ewdQ.elapsedTime(), 2);
      ...
      while (pid) {
        ...
        if (pid) {
          ...
          if (queuedRequest.action.exit && queuedRequest.action.pid) {
            pid = queuedRequest.action.pid;
            if (!ewdQ.process[pid].isAvailable) continue;
          }
          var requestNo = queuedRequest.action.requestNo;
          ...
          if (queuedRequest.action.exit) {
            delete ewdQ.process[pid];
            delete ewdQ.requestsByProcess[pid];
            if (ewdQ.traceLevel > 0) ewdQ.log("clearing down arrays for " + pid, 2);
          }
        }
        if (ewdQ.queue.length === 0) {
          pid = false;
          if (ewdQ.traceLevel > 0) ewdQ.log("queue exhausted", 2);
        }
      }
      childProcess = null;
      queuedRequest = null;
      if (ewdQ.traceLevel > 0 && ewdQ.queue.length > 0) {
  ...
  getChildProcessMemory: function(pid) {
    if (pid && !ewdQ.process[pid]) return;
    var requestObj = {
      action: {
        getMemory: true,
        requestNo: 'getMemory'
    ...
    if (pid) requestObj.action.pid = pid;
    ewdQ.addToQueue(requestObj,  function(requestObj, results) {
      //console.log("getChildProcessMemory - addToQueue callback back in ewdQ - results = " + JSON.stringify(results));
      ewdQ.sendMgrMessage({type: 'childProcessMemory', results: results, interval: ewdQ.monitorInterval});
      requestObj = null;
      results = null;
    });
    requestObj = null;
...
module.exports = {
  ...
  getChildProcessMemory: ewdQ.getChildProcessMemory,
