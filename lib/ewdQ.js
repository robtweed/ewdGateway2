...
var ewdQ = {
  ...
  memoryUsed: function() {
    ...
    return "rss: " + (mem.rss /1024 /1024).toFixed(2) + "Mb; heapTotal: " + (mem.heapTotal /1024 /1024).toFixed(2) + "Mb; heapUsed: " + (mem.heapUsed /1024 /1024).toFixed(2) + "Mb";
    mem = null;
  },
  ...
  getChildProcesses: function() {
    ...
    var pids = [];
    for (pid in ewdQ.process) {
      pids.push({pid: pid, available: ewdQ.process[pid].isAvailable})
    }
    return pids;
  },
     ...
     // *****************************************************
      ...
      ewdQ.process[pid].on('message', function(response) {
        ...
        if (response.ok) {
          ...
          if (!ewdQ.process[response.ok].started) {
  ...
  startChildProcess: function() {
    ...
    var childProcess;
    var pid;
    //var arg = [ewdQ.ewdQPath, ewdQ.database.type, ewdQ.database.nodePath, ewdQ.database.path, ewdQ.database.username, ewdQ.database.password, ewdQ.database.namespace, ewdQ.traceLevel, ewdQ.logTo, "", ewdQ.startTime, ewdQ.database.outputFilePath];
    var arg = [ewdQ.ewdQPath];
    childProcess = cp.fork(ewdQ.childProcessPath, arg);
    pid = childProcess.pid;
    ewdQ.process[pid] = childProcess;
    ...
    ewdQ.process[pid].on('message', function(response) {
      ewdQ.log("child process returned response " + JSON.stringify(response), 2);
      if (response.ok) {
