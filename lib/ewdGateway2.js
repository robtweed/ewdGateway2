  ...
  start: function(params, callback) {
    ...
    ewdQ.start(params, function() {
      ...
      var webserverCallback = function(request, response) {
        ...
        */
	    if ((request.method.toLowerCase() === 'post')&&(request.headers['content-type'].substring(0,20) === 'multipart/form-data;')&&(typeof ewdGateway.formHandler === 'function')&&(ewdGateway.formHandler(request,response))) {
          return;
        }
        var content = '';
          ...
          if (uri.substring(0,ewdGateway.onBeforeRenderPath.length)===ewdGateway.onBeforeRenderPath) {
            ...
            */
            ...
            onBeforeRenderResponse.once("ready", function(results) {
              ...
              console.log("onBeforeRenderResponse event triggered - results = " + JSON.stringify(results));
      ...
      if (ewdGateway.webSockets.enabled) {
        ...
        ewdGateway.socketClient = {};
        ewdGateway.socketClientByToken = {};
        var log = {};
        if (ewdGateway.traceLevel ===0) log = {log: false};
        ewdGateway.io = require(ewdGateway.webSockets.socketIoPath).listen(webserver, log);
        ewdGateway.io.set('log level', 0);
        if (ewdGateway.traceLevel > 0) ewdGateway.io.set('log level', 1);
        ewdGateway.io.sockets.on('connection', function(client){
          ...
          client.on('message', function(message){
            ewdGateway.log("Incoming websocket message from " + client.id + ": " + message, 1);
