...
module.exports = {
  ...
  onSocketMessage: function(ewd) {
    ...
    if (type === 'getGlobals') {
      var globalsArray = ewd.mumps.getGlobalDirectory({
        max: 25
      });
      var lastGlobalName = '';
      if (globalsArray.length === 25) {
        lastGlobalName = globalsArray[24];
      }
      ewd.session.$('lastGlobalName')._value = lastGlobalName;
      return globalsArray;
    }

    if (type === 'getNextGlobals') {
      var globalsArray = ewd.mumps.getGlobalDirectory({
        max: 25,
        lo: params.lo
      });
